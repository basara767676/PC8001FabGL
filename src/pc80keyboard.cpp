...
PC80KeyBoard::PC80KeyBoard() {}
PC80KeyBoard::~PC80KeyBoard() {}

uint8_t *PC80KeyBoard::mKeyMap;

int PC80KeyBoard::init(uint8_t *keyMap, void (*callBack)(void *, int), void *arg) {
    // PS2Controller
...
void PC80KeyBoard::reset() {
    mCaps = true;
    mKana = true;
    PS2Controller.keyboard()->setLEDs(false, false, false);
}

void PC80KeyBoard::suspend(bool value) {
    if (value) {
        PS2Controller.keyboard()->setLEDs(false, false, false);
    } else {
...
void PC80KeyBoard::run(void) { xTaskCreateUniversal(&keyBoardTask, "keyBoardTaskTask", 2048, this, 1, &mTaskHandle, APP_CPU_NUM); }
...
void IRAM_ATTR PC80KeyBoard::keyBoardTask(void *pvParameters) {
    auto kb = (PC80KeyBoard *)pvParameters;
                    ...
                    switch (scanCodeTable[scanCode].type) {
                        ...
                        case CTL_ALT_KEY:
                            if (!keyUp && (!kb->mLctrl || !kb->mRctrl) && (!kb->mLalt || !kb->mRalt)) {
                                ...
                                switch (scanCode) {
                                    case KEY_R:
                                        (*kb->mCallBack)(kb->mArg, CMD_BASIC_ON_RAM);
                                        delay(50);
                                        break;
                                    case INSERT:
                        ...
                        case SPECIAL_KEY:
                            switch (scanCode) {
                                ...
                                case F12:
                                    if (!keyUp) {
                                        kb->mSuspending = true;
                                        (*kb->mCallBack)(kb->mArg, CMD_PC80MENU);
                                    }
                                    break;
                                case F10:
                                    if (!keyUp) {
...
void IRAM_ATTR PC80KeyBoard::updateKeyMap(bool keyUp, int scanCode) {
    if (keyUp) {
        mKeyMap[scanCodeTable[scanCode].address] |= scanCodeTable[scanCode].data;
    } else {
        mKeyMap[scanCodeTable[scanCode].address] &= ~scanCodeTable[scanCode].data;
...
void PC80KeyBoard::setPadEnter(bool value) { mPadEnter = value; }